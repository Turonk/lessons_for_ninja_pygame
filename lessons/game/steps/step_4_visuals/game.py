...
RED = (255, 0, 0)
BLACK = (0, 0, 0)
# >>> ШАГ 4: новые цвета-заглушки для фонов (раньше их не было)
...
PICKUP_DISTANCE = 40  # Расстояние подбора снаряда
...
class AssetLoader:
    ...
    # >>> ШАГ 4: новый метод — загрузка фонов (раньше был только load_image)
    ...
    def load_all(self):
        """Загружаем все спрайты и фоны для игры."""
        # Спрайты персонажей
        ...
        # >>> ШАГ 4: загрузка спрайта врага (раньше враг рисовался как синий квадрат)
        self.sprites["enemy"] = self.load_image("assets/enemy.png", ENEMY_WIDTH, ENEMY_HEIGHT)

        # >>> ШАГ 4: загрузка фонов — небо и земля (раньше был fill(WHITE) + красная линия)
...
class Projectile:
    ...
    def update(self):
        ...
        if self.hit_surface and not self.stuck:
            ...
            # Трение: плавно замедляем горизонтальную скорость
    ...
    def draw(self, surface):
        ...
        if self.stuck:
            # Застрявший снаряд — приглушённый цвет, чтобы было видно разницу
...
# >>> ШАГ 4: достаём спрайт врага и фоны из AssetLoader (раньше этого блока не было)
...
while running:
    ...
    # --- Отрисовка фона ---
    # >>> ШАГ 4: было screen.fill(WHITE) + pygame.draw.line(красная линия)
    ...
    if spawn_timer >= SPAWN_DELAY and len(enemies) < MAX_ENEMIES:
        ...
        # >>> ШАГ 4: передаём спрайт (раньше было просто Enemy(en_x, en_y))
    ...
    # --- HUD: счётчик снарядов и врагов ---
    active_count = len([p for p in projectiles if p.active])
    font = pygame.font.Font(None, 36)
    # >>> ШАГ 4: цвет текста BLACK вместо (0, 0, 255) — синий плохо видно на голубом фоне
